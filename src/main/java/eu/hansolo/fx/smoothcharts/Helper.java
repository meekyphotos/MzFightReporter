...
public class Helper {
    ...
    public static int roundDoubleToInt(final double VALUE) {
        final double dAbs = Math.abs(VALUE);
    ...
    public static final Point2D[] subdividePoints(final Point2D[] POINTS, final int SUB_DEVISIONS) {
        ...
        final Point2D[] subdividedPoints = new Point2D[((noOfPoints - 1) * SUB_DEVISIONS) + 1];

        final double increments = 1.0 / (double) SUB_DEVISIONS;

        for (int i = 0; i < noOfPoints - 1; i++) {
            final Point2D p0 = i == 0 ? POINTS[i] : POINTS[i - 1];
            final Point2D p1 = POINTS[i];
            final Point2D p2 = POINTS[i + 1];
            final Point2D p3 = (i + 2 == noOfPoints) ? POINTS[i + 1] : POINTS[i + 2];

            final CatmullRom crs = new CatmullRom(p0, p1, p2, p3);
